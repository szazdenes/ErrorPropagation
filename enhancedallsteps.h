...
private:
    ...
    QList<int> initializeHist();
    QList<QVector2D> readHyperbolaFromFile(QString filename);
    double getError(double deg_of_pol, int stein);
    void writeDatFile(QString imname, QString hyperbola, QString timeOfDay, int stein, int p1_res, double p2_res, double deltoid_res, double second_error_res, double elev_res, QList<int> histogram, double north);
    ...
    QList<int> calculateNorthErrors(QList<QVector2D> sun_shadows, QList<QVector2D> hyp_points, double north);
